...
console.log(req.url);
  ...
  */
    ...
    eventemit.emit('fillName','7');
    
    
    
    //setTimeout(function(){eventemit.emit('blow2');},10) ;
   res.writeHead(200, {
    ...
    */    
       ...
       var doet = $(this).find('a').prop('title') +'  ('+timeToStringS+' - '+timeToStringE+')';
        ...
        if($('.schedule_details') == 'undefined'|| $('.schedule_details') == null){
          ...
          */ 
                   if(totCTime == 1620 || totCTime == 180||totCTime == 1500|| totCTime == 60||totCTime == 1560 || totCTime == 120){
  ...
  /*
    ...
    */    
    ...
    socket.on('liveServerRefresh',function(){
        var dd = new Array();
      var ff = 'zeenews/no/no show update';
        dd.push(ff);
        
      socket.emit('liveServerRefresh',dd[0]) 
    ...
    socket.on('dataOnLoad',function(dats){
      ...
      if(newsIdSave[0] == null){
           for(var j in dats){
        newsIdSave[j] = dats[j];
        console.log('lets seee -=-=-=-=- '+ newsIdSave[j]);
}
recursive(0);
}
   ...
   socket.on('initialize',function(result){
       ...
       socket.emit('userInfo',{userinfo:result.name});
      });
  ...
  */
         if(err2){
            ...
            console.log('error insurting the column' + err2);
              ...
              connection.query('UPDATE SUBSCRIPTION SET '+newsName+'_TIME'+'= NOW() + INTERVAL 5 SECOND ,' +newsName+'= ? WHERE USER_ID LIKE ?',[newsValue,idd],function(err3,results3){
                if(err3){
